<?php
class relProcessamentoAgua extends satecmax_pdf
	{
	...
	function setParametros($cdLeituraAguaMes)
		...
		$sqlHeader= "select 
								...
								a.total_consumido,	
							    (a.vlr_consumido + ifnull(vlr_consumido_1,0) + a.vlr_adicionais + (a.vlr_taxa_consumo * (select 
							            count(*)
							        from
							            tbl_unidades
							        where
	...
	function imprimirLinhaDetalhes($dadosLinha)
		...
		$this->totalCondominos += $dadosLinha["vlr_total"];
	 	}
	...
	function imprimeTotalCategoria($cdLeituraAguaMes)
		...
		//Quantidade de Unidades que participam do rateio de água
		$sqlUnidadesHidro = "select count(*) from tbl_unidades where st_hidrometro = 1";		
		$qtdeUnidadesHidro = P4A_DB::singleton()->fetchOne($sqlUnidadesHidro);
		...
		$this->cell(20,5,"R$ ".number_format($this->totalCondominos + 2 * $qtdeUnidadesHidro ,2,",","."),"BT",0,"R");
	...
	function Output()
		{
		return parent::Output("relProcessamentoAgua.pdf","S");
		}
