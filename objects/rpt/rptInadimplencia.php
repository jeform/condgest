<?php
class rptInadimplencia extends satecmax_pdf
	{
	...
	function setParametros($cdBoletoMes)
			...
			$sql = "SELECT 
					...
					WHERE
						    a.cd_boleto_mes = b.cd_boleto_mes
								...
								AND d.st_unidade = 1
					GROUP BY b.cd_unidade , b.dt_vencimento
					ORDER BY b.cd_unidade";
			...
			$sql = "SELECT
				...
				WHERE
					    a.cd_boleto_mes = b.cd_boleto_mes
					        ...
					        AND DATE_ADD(b.dt_vencimento, INTERVAL 1 DAY) < CURDATE()
							AND d.st_unidade = 1
							AND a.cd_boleto_mes = {$cdBoletoMes}
				GROUP BY b.cd_unidade , b.dt_vencimento
				ORDER BY b.dt_vencimento, b.cd_unidade";
		...
		$this->cell(70,5,utf8_decode("NOME"),1,0,"C");
		$this->cell(40,5,utf8_decode("MES/ANO REF."),1,0,"C");
		$this->cell(30,5,utf8_decode("VENCIMENTO"),1,0,"C");
		$this->cell(30,5,utf8_decode("VALOR"),1,0,"C");
	...
	function imprimirLinhaDetalhes($dadosLinha)
		...
		$this->cell(70,5,utf8_decode($dadosLinha["nm_pessoa"]),"BT",0,"C");
		$this->cell(40,5,utf8_decode($dadosLinha["mes_ano_referencia"]),"BT",0,"C");
		$this->cell(30,5,utf8_decode(formatarDataAplicacao($dadosLinha["dt_vencimento"])),"BT",0,"C");
		$this->cell(30,5,utf8_decode(number_format($dadosLinha["valor"],2,",",".")),"BT",0,"C");
	...
	function Output()
		{
		return parent::Output("rptInadimplencia.pdf","S");
		}
